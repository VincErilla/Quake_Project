...
Adafruit_LIS331HH accel1 = Adafruit_LIS331HH(); //the one with the tape that says 3
Adafruit_LIS331HH accel2 = Adafruit_LIS331HH();
...
const float A1_inv[3][3] = {
  {1.017613, -0.005011, -0.030121},
  {-0.005011, 1.056465, -0.036332},
  {-0.030121, -0.036332, 0.984441}
};

const float b1[3] = {-0.103845, 0.240050, -0.184010};

const float A2_inv[3][3] = {
  {1.028365, 0.026789, -0.017229},
  {0.026789, 1.039778, 0.028456},
  {-0.017229, 0.028456, 1.008058}
};

const float b2[3] = {0.011740, 0.037462, 0.007037};
...
void setup() {
  ...
  if (!accel1.begin_SPI(CS1, &spiAccel, 12345) || !accel2.begin_SPI(CS2, &spiAccel, 42069)) {
    Serial.println("Accelerometer init failed!");
    Serial.print("Accel 1: "); Serial.println(accel1.begin_SPI(CS1, &spiAccel, 12345));
    Serial.print("Accel 2: ");Serial.println(accel2.begin_SPI(CS2, &spiAccel, 42069));
    while (1) {
  ...
  accel1.setDataRate(LIS331_DATARATE_1000_HZ);
  accel2.setDataRate(LIS331_DATARATE_1000_HZ);
  accel1.setRange(LIS331HH_RANGE_12_G);
  accel2.setRange(LIS331HH_RANGE_12_G);
...
void loop() {
  ...
  if (now - lastSampleTime >= samplingIntervalMicros) {
    ...
    float x1_raw = e1.acceleration.x, y1_raw = e1.acceleration.y, z1_raw = e1.acceleration.z;
    float x2_raw = e2.acceleration.x, y2_raw = e2.acceleration.y, z2_raw = e2.acceleration.z;
